...
`_build_suggestions` is the one pure function on the read path: community windows in,
ranked `SuggestionItem`s out. Everything it depends on — the two Digital Twin fetches,
the committed-window lookup — is resolved by the caller and tested in
...
def test_window_visible_without_enrichment() -> None:
    """
    The community surplus is the event; the personal figures are decoration. A window
    with no per-device forecast behind it is still offered, with its personal fields
    null rather than zero — zero would read as "worth nothing to you", which is a
    different claim from "not known".
    """
...
def test_small_personal_impact_is_not_hidden() -> None:
    """
    There is no minimum. A window worth 0.08 kWh to this participant is still shown —
    the community benefit is the reason it exists, and a threshold here would hide the
...
def test_ranked_by_personal_impact_then_community_and_none_dropped() -> None:
    """
    Enriched windows first, because a participant can act on a personal number; the rest
    by community surplus. Nothing is dropped for lacking enrichment — that was the bug
    this ordering replaced.
    """
    windows = [
...
def test_enrichment_with_null_reward_points_does_not_crash() -> None:
    """
    A null reward with a real kWh figure collapses to 0 points rather than null, because
    the window *is* enriched — `reward_points is None` means "no forecast", and this one
    has one.
    """
    key = (datetime(2026, 7, 2, 9), datetime(2026, 7, 2, 12))
...
def test_confidence_absent_stays_none() -> None:
    """
    Confidence is a measured value or nothing. Defaulting it to 0 would render as "we
    are certain this is worthless", and defaulting it to 1 would be a lie.
    """
    window = _window()
...
def test_naive_window_timestamps_are_emitted_as_utc_aware() -> None:
    ...
    Emitting it naively made the frontend read it as local time (2h early in summer)
    while commitments — stored tz-aware — rendered correctly, so the same window
    appeared twice at different hours.
    """
    items = _build_suggestions([_window()], {}, set(), date(2026, 7, 2))
...
def test_clock_labels_use_italian_local_time() -> None:
    """
    09:00–12:00 UTC in July is 11:00–14:00 in Europe/Rome (CEST, +2). The machine-readable
    fields stay UTC and only the labels are localised, so the two never disagree about
    the instant.
    """
    items = _build_suggestions([_window()], {}, set(), date(2026, 7, 2))
...
def test_enrichment_lookup_survives_utc_normalisation() -> None:
    """
    Enrichment keys arrive naive from the per-device fetcher — still must match. Both
    sides are normalised before the lookup, because a miss here is silent: the window
    would be offered with no personal figures and look merely unprofitable.
    """
    key = (datetime(2026, 7, 2, 9), datetime(2026, 7, 2, 12))
...
def test_all_windows_visible_up_to_cap_of_eight() -> None:
    """
    Users must see every open window at once, not two at a time. Eight is a safety
    ceiling on a response size, not a curation rule — a day producing more than eight
    open windows would have some silently withheld.
    """
    windows = [
...
def test_a_malformed_window_is_skipped_not_fatal() -> None:
    """
    The rows are another service's output. One row missing `window_start` must cost that
    window and not the whole list — the alternative is a participant seeing nothing
    because one gold row was written badly.
    """
    windows = [
        _window("good"),
        {"_id": "bad", "community_kwh": 5.0},
        {"_id": "", "window_start": "2026-07-02T09:00:00", "window_end": "2026-07-02T12:00:00"},
    ]
    items = _build_suggestions(windows, {}, set(), date(2026, 7, 2))
